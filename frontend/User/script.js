    ...
    items: [
      {
        ...
        price: 25,
        description:
          "Traditional Indian spiced tea with aromatic blend of spices",
      },
      {
        id: "hb2",
        name: "Filter Coffee",
      ...
      },
      {
        ...
        price: 30,
        description: "Refreshing ginger-tulsi tea for immunity",
      },
      {
        id: "hb4",
        name: "Green Tea",
        price: 35,
        description: "Pure green tea leaves with natural antioxidants",
      },
      {
        id: "hb5",
        name: "Kesar Badam Milk",
        price: 60,
        description: "Rich saffron and almond flavored milk",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        name: "Ice Tea",
        price: 50,
        description: "Refreshing black tea served with ice and lemon",
      },
      {
        id: "cb3",
        name: "Lassi",
        price: 55,
        description: "Choose from Sweet, Salted, or Mango flavors",
      },
      {
        id: "cb4",
        name: "Nimbu Pani",
        price: 30,
        description: "Refreshing Indian lemonade with mint and spices",
      },
      {
        id: "cb5",
        name: "Mint Mojito",
        price: 65,
        description: "Fresh mint leaves muddled with lime and soda",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        price: 20,
        description:
          "Mumbai's favorite street food - spiced potato fritter in bun",
      },
      {
        id: "sb2",
        name: "Samosa",
        price: 25,
        description: "2 pieces of crispy pastry filled with spiced potatoes",
      },
      {
        id: "sb3",
      ...
      },
      {
        ...
        name: "Chole Bhature",
        price: 90,
        description: "Fluffy fried bread with spiced chickpea curry",
      },
      {
        id: "sb5",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        name: "Rabri",
        price: 65,
        description: "Condensed milk dessert with layers of malai",
      },
      {
        id: "d6",
        name: "Jalebi",
...
async function displayMenuItems(category) {
  ...
  const staticItems = menuCategories.find(
    (cat) => cat.title === category.title,
  ).items;
  const dynamicItems = dbMenuItems.filter(
  ...
  if (allItems.length === 0) {
    menuItemsContainer.innerHTML =
      '<p class="text-gray-500 text-center py-8">No menu items found.</p>';
    return;
  }
      ...
      html += `
        <div class="mt-6 border-t pt-4">
          ...
          <div class="mb-4 mt-6 flex items-center justify-between">
            <label for="table-number-input" class="text-sm font-medium text-gray-700">Table Number</label>
            <input id="table-number-input" type="text" placeholder="e.g., 5A" class="w-24 px-3 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-orange-700" />
          </div>
          <!-- End Table Number -->
      ...
      document.querySelectorAll(".quantity-decrease").forEach((btn) => {
        btn.addEventListener("click", () => {
          const item = cart.items.find(
            (i) => i.id === btn.getAttribute("data-id"),
          );
...
document.addEventListener("DOMContentLoaded", () => {
  initMenuCategories();
  updateCartDisplay();
  updateLoginBtn(); // Show correct Login/Logout state on page load
});
...
function openAuthModal(loginMode = true) {
  ...
  modalTitle.textContent = loginMode
...
function updateLoginBtn() {
  ...
  if (isLoggedIn()) {
    ...
    loginBtn.style.backgroundColor = "#b91c1c"; // solid red — visible on white nav
    loginBtn.style.color = "#fff";
  } else {
    loginBtn.textContent = "Login";
    loginBtn.style.backgroundColor = "#b45309"; // original orange-brown
    loginBtn.style.color = "#fff";
  }
}
  ...
  authForm.onsubmit = async function (event) {
    ...
    try {
      ...
      if (isLoginMode) {
        ...
        closeAuthModal();
        updateLoginBtn(); // Switch button to "Logout"
      } else {
...
async function checkout() {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        (await response.json()).error || "Order submission failed",
      );
